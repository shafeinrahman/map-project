...
-- Create the QUERY_LOG table to store user query records.
CREATE TABLE IF NOT EXISTS query_log ( -- Start table definition.
    query_id BIGINT PRIMARY KEY, -- Unique identifier for the query log.
    business_id BIGINT NULL, -- Optional reference to a business related to the query.
    poi_id BIGINT NULL, -- Optional reference to a point of interest related to the query.
    queried_at TIMESTAMPTZ, -- Timestamp when the query was made.
    queried_by BIGINT, -- Reference to the user who made the query.
    metadata JSONB, -- Additional structured metadata for the query.
    CONSTRAINT fk_query_log_business FOREIGN KEY (business_id) REFERENCES business(business_id), -- Enforce optional business reference.
    CONSTRAINT fk_query_log_poi FOREIGN KEY (poi_id) REFERENCES point_of_interest(poi_id), -- Enforce optional point of interest reference.
    CONSTRAINT fk_query_log_queried_by FOREIGN KEY (queried_by) REFERENCES "user"(user_id) -- Enforce querying user reference.
); -- End table definition.
...
CREATE TABLE IF NOT EXISTS business ( -- Start table definition.
    ...
    address_text TEXT, -- Physical address of the business.
    ...
    last_queried_at TIMESTAMPTZ, -- Timestamp when the business was last queried.
    ...
    CONSTRAINT fk_business_route FOREIGN KEY (route_id) REFERENCES route(route_id), -- Enforce route reference.
    CONSTRAINT fk_business_territory FOREIGN KEY (territory_id) REFERENCES territory(territory_id), -- Enforce territory reference.
...
-- Create the POINT_OF_INTEREST table to store points of interest.
CREATE TABLE IF NOT EXISTS point_of_interest ( -- Start table definition.
    poi_id BIGINT PRIMARY KEY, -- Unique identifier for the point of interest.
    poi_type TEXT, -- Type of the point of interest.
    poi_name TEXT, -- Name of the point of interest.
    created_at TIMESTAMPTZ, -- Timestamp when the point of interest was created.
    created_by BIGINT, -- Reference to the user who created the point of interest.
    last_queried_at TIMESTAMPTZ, -- Timestamp when the point of interest was last queried.
    latitude NUMERIC(9,6), -- Point of interest latitude coordinate.
    longitude NUMERIC(9,6), -- Point of interest longitude coordinate.
    CONSTRAINT fk_point_of_interest_created_by FOREIGN KEY (created_by) REFERENCES "user"(user_id) -- Enforce creator user reference.
); -- End table definition.
